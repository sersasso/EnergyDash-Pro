...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s %(levelname)s energydash-daily %(message)s",
)
logger = logging.getLogger("energydash-daily")
...
def update_daily(engine, cfg, days):
    ...
    query = text("""
        ...
        FROM measures_raw
        WHERE ts >= :since
          AND prod_energy_wh IS NOT NULL
          AND cons_energy_wh IS NOT NULL
        ORDER BY ts
    """)
    ...
    if daily.empty:
        logger.info("Nessun dato giornaliero da aggiornare")
        return 0

    upsert = text("""
        INSERT INTO energy_daily(
            day, production_wh, consumption_wh, grid_import_wh, feed_in_wh,
            self_consumption_wh, source, quality, samples, is_anomaly,
            anomaly_note, updated_at
        )
        VALUES(
            :day, :production_wh, :consumption_wh, :grid_import_wh, :feed_in_wh,
            :self_consumption_wh, 'shelly', 'computed_interval', :samples,
            false, NULL, now()
        )
    ...
    logger.info("Giorni aggiornati: %s", len(daily))
    return len(daily)


def cleanup_raw(engine, cfg):
    retention_days = int(cfg.get("retention", {}).get("raw_days", 180))

    if retention_days <= 0:
        logger.info("Retention raw disabilitata")
        return 0

    sql = text("""
        DELETE FROM measures_raw
        WHERE ts < now() - (:days || ' days')::interval
    """)

    with engine.begin() as conn:
        result = conn.execute(sql, {"days": retention_days})
        deleted = result.rowcount or 0

    logger.info(
        "Retention measures_raw: eliminati %s record piu vecchi di %s giorni",
        deleted,
        retention_days,
    )
    return deleted
...
def main():
    ...
    update_daily(engine, cfg, args.days)

    if not args.skip_retention:
        cleanup_raw(engine, cfg)
